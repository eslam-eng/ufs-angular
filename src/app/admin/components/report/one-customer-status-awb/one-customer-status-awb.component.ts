...
@Component({
  ...
  templateUrl: './one-customer-status-awb.component.html',
  styleUrls: ['./one-customer-status-awb.component.scss']
})
export class OneCustomerStatusAwbComponent  implements OnInit {

  baseApiUrl: any = "report/awb-prices";
  title: any = "one_customer_awb_status_report";
  status: any = [];
  breadcrumbList: any = [];
  companies: any = [];
  filter: any = {};
  ...
  loadChartData() {
    this.service.get('report/one-company-awb-status', this.filter).subscribe((res:any) => {
      this.response = res;
